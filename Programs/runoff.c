...
void tabulate(void)
{
    //Base case: only one candidate. First create loop to count candidates
    ...
    //Base case: only one candidate remains
...
//Loop over non-eliminated candidates to count votes.
        for (int i = 0; i < voter_count; i++)
        {
...
*/
/*
        //return candidates vote count to 0
        for (int j = 0; j < voter_count; j++)
        {
           candidates[j].votes = 0;
        }

        //recursion: re-count until winner
        tabulate();
*/
    }
}
